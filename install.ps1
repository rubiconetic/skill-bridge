...
# Create sb.ps1 wrapper so PowerShell can invoke the bash script
$sbWrapper = "$BIN_DIR\sb.ps1"
$sbScript  = "$INSTALL_BASE\bin\sb"
Write-Info "Creating sb.ps1 wrapper at $sbWrapper..."

# Find bash — prefers Git for Windows, falls back to WSL
$bashExe = $null
$candidatePaths = @(
...
if (!$bashExe) {
    Write-Warn "bash not found. 'sb' wrapper will not work until Git for Windows is installed."
    $bashExe = "bash"  # best-effort fallback
}

# Escape backslashes for the here-string
$bashExeEscaped = $bashExe -replace '\\', '\\'
$sbScriptEscaped = $sbScript -replace '\\', '/'

$wrapperContent = @"
# Auto-generated by install.ps1
& '$bashExeEscaped' '$sbScriptEscaped' @args
"@
$wrapperContent | Out-File -FilePath $sbWrapper -Encoding utf8 -Force
